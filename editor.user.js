...
var main = function () {
    ...
    //within the same function scope, we can now use simple names appended to a namespace object.
    ...
    EM.replaceCode = function (str, type) {
        for (var i in EM.replacedStrings[type]){
            str = str.replace(EM.placeHolderChecks[type], EM.replacedStrings[type][i]);
        }
        return str;
    }
        ...
        for (var j in EM.edits) {
            if (EM.edits.hasOwnProperty(j)) {
                ...
                if (fix) {
                    ...
                    //insert results
                    EM.reasons[numReasons] = fix.reason;
